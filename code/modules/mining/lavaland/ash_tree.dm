...
/obj/structure/flora/ashtree/Initialize(mapload)
	...
	//Random coal or wood amount, so its not bog standard.
...
//So we dont lose our bowls, stolen from closet code
...
/obj/structure/flora/ashtree/proc/harvest_sap()
	...
	if(RG.is_refillable())
		//Make sure that its not filling something that's full
		if(!RG.reagents.holder_full())
			RG.reagents.add_reagent(sap_type, min(RG.volume - RG.reagents.total_volume, sap_amount))
	//We drop our used beaker and try to fill it with sap
...
//If you hit it with a sharp force above 0 item it chops it down, unlike trees tho it dosnt give wood as it's already charcoal
//Also doesn't have a stump
/obj/structure/flora/ashtree/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/reagent_containers))
		if(sap)
			...
			sap = FALSE
			container_used = W
			//So we dont lose our bowl when cutting it down + needed for the harvest sap proc
